...
class PatientQueue {
      ...
      patients.get(parent(i)).getLevel() < patients.get(i).getLevel() ||
        (
          ...
          patients.get(parent(i)).getOrder() > patients.get(i).getOrder() 
			...
			map.put(temp.getName(), parent(i));
			map.put(patients.get(parent(i)).getName(), i);
      ...
      int maxO = patients.get(i).getOrder();
        ...
        (
          ...
          maxL == patients.get(left(i)).getLevel() && maxO > patients.get(left(i)).getOrder()
				...
				maxL = patients.get(left(i)).getLevel();
				max_id = left(i);
        maxO = patients.get(left(i)).getOrder();
			}
			if (right(i) <= len &&
        (
          ...
          maxL == patients.get(right(i)).getLevel() && maxO > patients.get(right(i)).getOrder()
				...
				maxL = patients.get(right(i)).getLevel();
        maxO = patients.get(right(i)).getOrder();
				max_id = right(i);
			}
			if (max_id != i) {
				Patient temp = patients.get(i);
				map.put(temp.getName(), max_id);
				map.put(patients.get(max_id).getName(), i);
  ...
  public Patient remove(String patientName) {
	...
	private Patient extract(int index) {
		Patient target = patients.get(index);
		map.put(patients.get(len).getName(), index);
		patients.set(index, patients.get(len));
		len--;
		shiftup(index);
		shiftdown(index);
		return target;
	...
	public void update(String patientName, int patientLevel) {
    int idx = map.get(patientName);
    Patient p = patients.get(idx);
    p.setLevel(patientLevel+p.getLevel());
    shiftup(idx);
	}
}